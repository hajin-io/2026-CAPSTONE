...
COM 포트 번호는 윈도우가 **USB 슬롯 + 아두이노 조합**에 부여하는 번호입니다.
기본 상태에서는 다른 슬롯에 꽂으면 번호가 바뀔 수 있으므로 아래와 같이 고정합니다.

1. **장치관리자** 열기 (Win + X → 장치관리자)
2. **포트(COM & LPT)** 펼치기
3. 아두이노 항목 **우클릭 → 속성**
4. **포트 설정** 탭 → **고급** 버튼 클릭
5. **COM 포트 번호** 드롭다운에서 원하는 번호 선택 후 확인

고정 후에는 어느 슬롯에 꽂아도 해당 아두이노는 항상 같은 COM 번호로 인식됩니다.
